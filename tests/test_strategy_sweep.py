...
class StrategySweepTests(unittest.TestCase):
    def test_ranks_candidate_results(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            results = run_strategy_sweep(config, storage, [market], candles, "all", 3)
    ...
    def test_candidate_limit_caps_evaluated_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            results = run_strategy_sweep(config, storage, [market], candles, "all", 10, candidate_limit=2)

            self.assertEqual(len(results), 2)

    def test_max_points_per_market_limits_sweep_history(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            config = AppConfig(
                api=ApiConfig("", "", 1),
                cache=CacheConfig(False, ".cache/http", 60, False),
            ...
            results = run_strategy_sweep(config, storage, [market], candles, "all", 1, candidate_limit=1, max_points_per_market=10)

            self.assertEqual(results[0].market_count, 0)
            self.assertEqual(results[0].trade_count, 0)
