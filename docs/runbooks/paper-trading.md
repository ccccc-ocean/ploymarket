...
`paper-run` 是实时模拟扫描，不能把任意旧 SQLite 缓存当作可交易依据。现在它优先使用 live market discovery；如果 Polymarket/Gamma 实时发现短暂抖动，只允许回退到最近 `fresh_market_ttl_seconds` 内 live 观察过的市场池。价格历史仍必须实时拉取，失败的市场会跳过，不用旧价格补位。
