...
- 每个 `TAKER` 候选还会写入 `execution_stress_<timestamp>.csv` 与 `shadow_order_events_<timestamp>.csv`，影子模拟延迟价格恶化、部分成交和操作失败；`execution_stress_report.csv` 累计汇总这些结果。报告不发送订单，也暂不改写主 paper PnL。
...
- `execution_stress` 中候选在延迟价格恶化后是否仍通过，若理论 TAKER 多但 `robust` 长期为零，则不能进入实盘；部分成交撤残单需单独观察其发生频率。
