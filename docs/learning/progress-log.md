...
## 2026-05-26：将执行压力从硬拒单改为并行成交路径

### 对 K 线数据的澄清

- VPS `btc-price` 正常每轮使用 Coinbase live 请求，明确传入 `use_cache=false`；VPS 配置本身也关闭 HTTP cache。
- 若当轮 live 请求失败，程序会短暂沿用磁盘中上一轮已实时取得的 candle；当前只要最新 candle 时间桶起点距本轮不超过 `15` 分钟即可用于方向性判断。
- 五分钟粒度实测最新 candle 时间桶起点距查询时约 `270` 秒，说明正常运行时使用的是当前五分钟桶。`15` 分钟是断流保护阈值，不是未来实盘所需的最佳新鲜度标准；后续应增加更即时 spot/ticker 确认并收紧开仓回退。

### 执行模拟修正

- 用户指出固定最坏延迟场景不应直接压低策略成交样本，这一判断正确：真实交易可能立即成交、完全不成交或部分成交，需要分别评估。
- 主 paper 路径恢复为：策略通过实时 ask 与基础 edge 规则后，记录基线模拟成交，不再由固定 `+0.01` 压力情景直接拒掉。
- 压力层继续独立记录价格恶化结果，并新增 `fok_unfilled_no_liquidity` / `CANCELED_UNFILLED` 场景，表示 `FOK` 因深度不足而整笔没有成交。
- 已有 `FAK` 部分成交撤余单、撤单未确认、签名/余额失败场景继续保留。下一步应建立多条执行路径各自的 PnL 账本，而不是只报告事件计数。

### 判断

这次调整使五分钟链更接近真实交易研究：交易意图和实时 ask 基线不会被极端假设提前消失，同时执行摩擦仍被清楚记录。账户级实盘风控和跨轮订单对账仍需补齐，当前不建议实盘。

## 2026-05-27：修复静默停跑并增加 VPS 自愈监控

### 昨夜暴露的问题

- VPS 的 `btc-price` 仍能输出，但 `paper-run` 因持仓分类路径缺少导入而从北京时间 `2026-05-27 01:55` 起持续失败；仅看部分日志会误认为实时链仍在正常采样。
- 当时脚本只有标准输出、错误日志和简单锁，没有可机器判断的成功状态，也没有失败后重试或遗留锁修复，因此故障直到人工检查才被发现。
- 修复导入后，恢复运行立即对延迟未处理的 `May 27 above 76k BUY_YES` 触发止损，已实现亏损约 `-12.60 USDC`。这说明停跑会直接扭曲 PnL 与风险暴露判断。

### 已增加的保护

- 实时链与深度链每轮写入原子健康状态，明确记录 `running`、`success`、`failed`、最后步骤和最后成功时间。
- 新增 watchdog 定时检查：实时链超过 `10` 分钟无成功或超过 `4` 分钟未完成会重试；研究链对应阈值为 `2` 小时和 `1` 小时。
- 步骤执行增加超时限制，且异常退出遗留的无主锁会自动清理，恢复动作写入 `watchdog_events.csv` 便于审计。
- `daily-report` 现在把实时链健康作为硬门槛：链路失败、缺失或过期时保持 `not_ready`，不允许使用不完整样本支持实盘决策。

### 边界与下一步

watchdog 可以自行恢复短暂网络故障、任务卡死和遗留锁，但不能安全地凭空修复持续存在的策略或代码错误；此类问题会快速显式暴露并阻止实盘判断，再由代码修复和验证闭环处理。当前策略 PnL 与数据可靠性都不足以支持实盘，仍需继续在健康 VPS 链路上积累前瞻样本。

## 2026-05-27：空实时轮次不再算作健康运行

### 下午发现的问题

- 北京时间 `17:55` 与 `18:00`，VPS 的 Gamma 市场发现连续返回 `403 Forbidden`，两轮 `paper-run` 与 `spread-scan` 只能输出 `markets=0` / `DATA_DEGRADED`。
- 接口随后在 `18:05` 自行恢复到 `49` 个实时市场，说明更接近临时 API/Cloudflare 拒绝，而不是永久断网；但旧健康状态会把空轮仍写成 `success`，这会高估有效采样连续性。
...
- 修复已有仓位监控遗漏：`paper-run` 现在会把不在当轮 live discovery 列表中的 open position 纳入退出检查，但这些本地识别的持仓不会形成新的开仓候选。该问题在 BTC 跌破 `75k` 后核对持仓时暴露，`dip to 74k May 25-31` 此前未进入最近轮次监控。
- Gamma 旧分页端点已返回弃用提示，官方推荐 `/markets/keyset` cursor 分页；初步探测中直接携带现有排序参数迁移会出现服务端错误，因此暂不仓促替换，后续需做市场覆盖对照后上线。

### 判断

空轮保护提高的是结果真实性，并不会改善策略本身。下午模拟盘还出现 `BUY_NO above 76k` 止盈后重新开仓再止损的路径，说明 `price_range_daily` 的再入场规则仍需要收紧，当前继续保持 `not_ready`。
